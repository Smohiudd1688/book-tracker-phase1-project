...
function findBooks() {
    ...
    .then(response => response.json())
    .then(object => addBookToList(object));
}

function addBookToList(object) {
    ...
    const titleE = document.createDocumentFragment('h3');
    titleE.textContent = title;
    //title.setAttribute('class', 'book');

    const coverE = document.createElement('img');
    coverE.src = cover;
    coverE.setAttribute('class', 'book-cover');

    const authorE = document.createDocumentFragment('p');
